// FIXME: Solver code duplicated to function in worker. 
// A cleaner solution would be using worker-pluigin with webpack so the module could be sharable between the main and worker threads.
importScripts("https://unpkg.com/heap-js@2.1.2/dist/heap-js.umd.js");
...
function solver(sudokuStr) {
  ...
  function backtrack(grid) {
    ...
    for (let row = 0; row < 9; row++) {
      for(let col = 0; col < 9; col++) {
        if (grid[row][col] === '.') {
          ...
          heap.Heap.heappush(emptyCellHeap, [row, col, cellPossibleAnswers], comparator)
        }
      }
    }
    ...
    while (emptyCellHeap.length) {
      const prioritizedAnswer = heap.Heap.heappop(emptyCellHeap, comparator)
